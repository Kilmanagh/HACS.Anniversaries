"""Sensor platform for Anniversaries (repaired).
Clean rewrite to avoid hidden indentation artifacts. File version marker below
allows users to verify they have the expected sanitized copy.
"""
__FILE_VERSION__ = "2025-08-13-2"
...
class AnniversarySensor(CoordinatorEntity[AnniversaryDataUpdateCoordinator], SensorEntity):
    ...
    @property
    def native_value(self) -> int | None:  # noqa: D401
        ann = self.anniversary
        return ann.days_remaining if ann else None

    @property
    def icon(self) -> str:  # noqa: D401
        ann = self.anniversary
        if not ann:
...
class UpcomingAnniversariesSensor(CoordinatorEntity[AnniversaryDataUpdateCoordinator], SensorEntity):
    ...
    @property
    def native_value(self) -> str | None:  # noqa: D401
